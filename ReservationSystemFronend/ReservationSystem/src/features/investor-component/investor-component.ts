...
export class InvestorComponent implements OnInit {
  ...
  investors = signal<User[]>([]);
  topics    = signal<Topic[]>([]);
  isLoadingInvestors = signal(false);
  ...
  // ── Form ─────────────────────────────────────────────────────────────────────
  investorForm: InvestorForm = this.blankForm();
  ...
  resetForm(): void {
    this.investorForm = this.blankForm();
  ...
  // ── Topic rows ───────────────────────────────────────────────────────────────
  addTopic(): void    { this.investorForm.topics.push(this.blankTopic()); }
  removeTopic(i: number): void { this.investorForm.topics.splice(i, 1); }
  ...
  // ── Validation ───────────────────────────────────────────────────────────────
  isFormValid(): boolean {
    if (!this.investorForm.name.trim())   return false;
    if (!this.investorForm.mobile.trim()) return false;
    if (this.investorForm.topics.length === 0) return false;
    return this.investorForm.topics.every(t =>
      t.topicId && t.year && t.month && t.day && t.fromHour && t.toHour
    );
  }

  hasTimeIntersection(): boolean {
    const topics = this.investorForm.topics;
  ...
  saveInvestor(): void {
    ...
    const payload: UserDto = {
      ...
      mobile:   this.investorForm.mobile,
      userType: 2,
      userTopicAvaliabilityDTOs: this.investorForm.topics.map(t => ({
        topicId:          t.topicId!,
    ...
    this.userService.addUser(payload).subscribe({
      next: () => {
        console.log('Investor saved:Fired');
        this.isSaving.set(false)  ;
        this.successMsg = 'Investor saved successfully.';
        this.resetForm();
      },
      error: (err) => {
        this.isSaving.set(false)  ;

        this.errorMsg = 'Failed to save investor. Please try again.';
        console.error(err);
      },
    });
  }
  ...
  loadInvestors(): void {
    ...
    this.userService.getAllInvestors().subscribe({
      next: (data: any) => { this.investors.set(data); this.isLoadingInvestors.set(false); console.log(this.investors()); },
      error: ()         => { this.isLoadingInvestors.set(false); },
    });
  }
